...
const tutorialSteps = [...html.matchAll(/<article class="tutorial-step(?: active)?" data-step="(\d)">([\s\S]*?)<\/article>/g)];
assert.equal(tutorialSteps.find(([, step]) => step === "0")?.[2].includes('id="tutorialDemo"'), false, "Demo audio must not appear at the start of the tutorial");
assert.equal(tutorialSteps.find(([, step]) => step === "4")?.[2].includes('id="tutorialDemo"'), true, "Demo audio must appear at the end of the tutorial");
