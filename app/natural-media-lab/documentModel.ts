...
export const parseProject = (value: unknown): NaturalMediaDocument => {
  ...
  const fallbackPageId = crypto.randomUUID();
...
const openDatabase = () => new Promise<IDBDatabase>((resolve, reject) => {
  const request = indexedDB.open("natural-media-lab", 1);
  request.onupgradeneeded = () => request.result.createObjectStore("projects");
  request.onsuccess = () => resolve(request.result);
  request.onerror = () => reject(request.error);
});

export const saveRecovery = async (document: NaturalMediaDocument) => {
  const database = await openDatabase();
  await new Promise<void>((resolve, reject) => {
    const transaction = database.transaction("projects", "readwrite");
    transaction.objectStore("projects").put(document, "autosave");
...
export const loadRecovery = async () => {
  const database = await openDatabase();
  const result = await new Promise<NaturalMediaDocument | undefined>((resolve, reject) => {
    const request = database.transaction("projects").objectStore("projects").get("autosave");
    request.onsuccess = () => resolve(request.result);
    request.onerror = () => reject(request.error);
  });
  database.close();
