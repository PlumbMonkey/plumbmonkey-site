...
- Progress through checkpoint 12: rotated manipulation, project/package contracts, versioned document and raster recovery, the shared document/command core, verified binary checkpoints, raster-surface adapters, a handle-backed live raster session and working-document v1 projection, dirty-tile persistence/restoration, reversible tile-revision commands, unified mixed Undo/Redo history, bounded in-memory journal compaction, shared document snapshots, animation interpolation/timing/rendering, shared rig math, canvas operations, unified comic rendering, export planning, workload profiling, deterministic generator/encoder tests, and the destructive-edit/forced-restart regression gate are complete.
...
4. **In progress:** extract viewport, document, commands, persistence, painting, rigging, comics, and export services from the React component. The document model, recovery, commands, binary/checkpoint storage, raster geometry, handle-backed raster session and working-document projection, incremental tile persistence/restoration, revision commands, mixed raster-aware history controller, compatibility snapshot boundary, animation interpolation/timing/rendering, rig math, canvas operations/compositing, comic layout/rendering, export planner, and workload profiler are now separated. The remaining Phase 0B work is adopting the working projection at persistence boundaries and extracting export-job coordination.
