...
### Done 2026-09-15 (all six items; uncommitted)
Gregg's answers: Beam gets a *real* survival round; Amp's barrel throwing and clunky throw animation go, replaced by **toppling speaker stacks**; the key and cage go, replaced by **plugging in three leads**; Graveyard's lantern tally pays **notes + time + ghosts**.
1. **Beam Survival Round** (`beam.js`, `beam-data.js` `SURVIVAL`, `beam-art.js`). Kind `"survival"`: the 40 flyers loop their patterns (every pattern starts and ends off-screen) and shoot back (bolt cap 3–5, 0.3%/frame while on-screen) for a 30 s clock (`SURVIVE` = 1800 frames of play; it pauses while you're down). Time up → bolts cleared, flyers `flee`. Bonus = 100 per second since your last loss (clearing all 40 early banks the rest of the clock) + 10,000 UNTOUCHED if no ship was lost. HUD shows `SURVIVE 24s · HITS n`. Tests: flyers fire and loop, time-up flee, untouched bonus, a loss resets the streak.
2. **Cruise orange box**: two culprits, both gradients that never reached transparency inside their fill: `lamp()` in `cars.js` (a wide tail-light bar got a round halo sized to its *width* clipped to a 39px box, so a translucent red-orange rectangle sat behind the car) and the underglow in `render.js`. Both are now ellipses (scaled radial gradients) that fade out inside their box. Verified in the pane.
3. **Cruise billboards**: `BILLBOARD_GAMES` in `tracks.js`, one billboard variant per other cabinet (roadside `seed` rotates them). The board paints the title strip first, then draws `billboards/<slug>.jpg` in place once it loads (480×270 stills from each cabinet's attract mode). Capture trick for the hidden pane (it throttles timers and only runs a few frames per screenshot): fake `performance.now`/`Date.now`, step the game 720 frames synchronously, and POST `canvas.toDataURL` to a tiny local receiver (a 20-line Python server; Python's `http.server` has no POST). Wave-3 cabinets: wrap `window.requestAnimationFrame` to record `wave3.js`'s loop callback, take one screenshot so a frame fires, then call the recorded callback. The standalone cabinets schedule through `ArcadeVR.schedule`, which holds its own `realRaf`, so a window shim never sees them; instead make `ArcadeVR.schedule` a no-op and call their global `loop(now)` directly. To refresh a still, rerun this after a game's art changes.
4. **Amp Rampage redesign** (`amp-data.js`, `amp-stages.js`, `amp.js`, `amp-art.js`):
   - **Load-in**: no barrels, no drum. Plumbmonkey paces x 60–260 on truss 5 and **stomps** (`stomp` pose, knee up over `KICK_WIND` 42 f). The stomp sends a lime **pulse** (`ORB` 20 f) to the standing stack whose crash zone is nearest the Spaceman (his truss, or the one above). The stack **wobbles** `WOBBLE` 60 f with a red band growing along the walkway from its foot and an ember arc showing where its top lands, then **crashes** across `zone(k)` (20 px behind the foot to 96 px ahead): fatal inside, a 100-pt close call within 80 px. A crash over a ladder top sends a **cabinet tumbling down that ladder** (fatal at the foot, shatters at the bottom). Every 5th stack is hot and sparks a fire ghost. Wreck lies `DOWN` 150 f, gone `REBUILD` 240 f, rises `RISE` 40 f. The mic stand smashes standing, armed or wobbling stacks and tumbling cabinets. Stack spots are `LOADIN.stacks` `[girder, x, dir]`. The autopilot pays a 1500 penalty for ending a plan inside a lit zone.
   - **Cable Jungle**: no key, no cage. Three leads (`CABLES.leads`) and three sockets (`CABLES.sockets`); carry one lead at a time, any lead fits any socket, each plug scores 800, and the third powers the stage (clear). Plugged sockets survive a death (`carryOf` → `plugged`). HUD: POWER pips. `CABLE_ROUTE` has `lead`/`plug` waypoints.
   - Stage Build lost its cage too. Tests: stomp → pulse → wobble → crash, close call, tumbling cabinet (hit and miss), hot stack, rebuild, hammer vs stack, leads + sockets + respawn; the bot clears two loops.
5. **Graveyard great lantern**: `E` is now a great lantern standing on the ground below it (`s.exitLanterns`; the whole column above it lights it, so it can't be jumped over). Touch → `light` state, a spark carries from his hand to the wick over `LIGHT` 40 f, then a flare, a bloom and the tally banner `NOTES n × 50 · TIME s × 20 · GHOSTS g × 100`. Adds a **level clock** (`TIME` 300 s, frozen inside a boss arena; HURRY at 60 s; time-out costs a life) and level stats (`s.stats`, kept through a respawn, fresh each level). `paintGreatLantern` replaces `paintBell`. Tests: column trigger, tally, clock, arena freeze, stats.
6. **Revenger hero death**: `FX.shipDeath(x, y)` in `fx.js` (the old 180 + 60 rainbow particles with friction, a ring racing to 500 px, a 0.7 white flash, a 40-frame shake), layered on the existing `FX.explode(…, 3)` in `damageShip()` and also used for SPACEMAN DOWN in `runner.js`. FX-only, so the FX-on/off identity test still passes.

### Done 2026-09-15, round 2 (Gregg's next three notes; uncommitted)
1. **Infestation was far too easy** (he "collected so many lives"):
   - Extra lives: the first at **25,000**, then every **50,000**, and **never more than 5** in hand (`FIRST_EXTRA_LIFE`, `EXTRA_LIFE_EVERY`, `MAX_LIVES` in `game.js`). It was every 12,000 with no cap.
   - The Hauntipede is faster everywhere: `2.3 + 0.12 × level`, capped at 4.2 (was 2.1 + 0.1, cap 3.7).
   - **The Conservatory is the hardest ground**: `speedMult 1.2` (Hauntipede), `spawnMult 1.45` (critters come back faster), puffballs burst into a fan of **3** spores (`spores: 3`), longer Hauntipedes (14/15/16), denser beds (46/52/58 toadstools, 4/5/6 flytraps, puffball share 35/42/50%), bigger rosters (up to 4 ghosts, 4 bugs, 3 spiders, 2 beetles, 2 scorpions). The boss arena has 30 toadstools, 3 traps, 45% puffballs and a spider/scorpion escort.
   - **Mandrake**: HP 22 → 28, vine warning 50/40 → 42/34 frames, open window 100/85 → 84/70, seed volleys 6/8 → 7/9. Enraged still adds the second vine.
   - The autopilot now steps out of warned or slamming vine columns (all but two of its Mandrake deaths were vines). Unassisted probe: 4-1 to 4-3 clear with 0–1 deaths; the Mandrake clears in 30–100 s with 0–4 deaths.
2. **Mess Hall and Swarm: no on-screen targeting visuals.** Removed Mess Hall's dashed aim guide and cursor ring, Swarm's reticle and muzzle sight, Mess Hall's `cursor: crosshair` (now `default`), and the gamepad DOM reticle in `leaderboard.js` (the virtual aim cursor still moves, it just isn't drawn). The arm and rifle show the aim. Enemy attack tells (a monster's dashed throw/pounce line) were kept. Versions: leaderboard `?v=8` on all 12 pages, Mess Hall render `?v=5`, Swarm render `?v=6`.
3. **The Spaceman in Midnight Cruise.** `drawDriver('hero')` in `cars.js` is now the Spaceman from behind in the Hero Kit palette (long dark swept-back hair, white quilted suit, navy vest straps, neck ring, gold aviator arms). The cabinet loads `../kit/hero-kit.js?v=5`, and the finish banner draws `HeroKit.spaceman` beside it (victory pose on a win, idle otherwise). The intro copy now names him.
4. **The Spaceman rides Luno.** The Hero Kit gained a `ride` pose (thighs forward, shins hanging, both hands on the reins, a little bob from `phase`). `drawLuno` in `spectral-skyline/luno.js` draws `HeroKit.spaceman(…, { pose: 'ride', scale: 0.62, tilt })` with his hip on the violet saddle, and keeps the old helmeted rider only as a fallback if the kit isn't loaded. The cabinet loads `../kit/hero-kit.js?v=6` (`luno.js?v=4`), and `scripts/test-luno.cjs` loads the kit too. Other cabinets keep their kit versions; the new pose is additive.

### Change list (as written 2026-09-14)
